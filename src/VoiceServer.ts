...
export default class VoiceServer {
  ...
  static init(bot: Bot) {
    ...
    this.udpClient.on("error", (err) => {
      throw new Error(`Failed to connect to UDP server: ${err}`);
    });
  ...
  private static async handler(msg: Buffer, _: dgram.RemoteInfo) {
    const packet = PacketManager.protoDef.parsePacketBuffer(
    ...
    if (packet.data.id == "PingPacket") {
      ...
      if (
        PacketManager.sourceById.some((item) =>
        ...
        this.bot.emit("voicechat_voice", {
          ...
          data: this.opusEncoder.decode(
            await PacketManager.decryptVoice(data.data)
          ),
        });
      } else {
  ...
  static async sendPCM(pcmBuffer: Buffer, distance: number = 16) {
    ...
    const frameSize =
      (PacketManager.configPacketData.captureInfo.sampleRate / 1_000) * 20;
    ...
    for (let i = 0; i < frames.length; i++) {
      ...
      const opus = this.opusEncoder.encode(frame);
      ...
      const encodedVoicePacket = await PacketManager.encodeUDP(
        voicePacket,
        "PlayerAudioPacket",
        this.udpSecret
      );
